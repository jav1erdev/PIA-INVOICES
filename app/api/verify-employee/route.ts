...
function compareFaceDescriptors(inputDescriptor: Float32Array, dbDescriptor: Float32Array) {
  const distance = faceapi.euclideanDistance(inputDescriptor, dbDescriptor);
  return distance < 0.6;
}

async function handleWorkSchedule(action: string, employeeId: number, now: string) {
  if (action === 'entry') {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const matchedEmployee = employeeCache.find(emp => compareFaceDescriptors(inputFaceDescriptor, emp.descriptor));
    
    if (!matchedEmployee) {
      return NextResponse.json({ message: '❌ No se encontró coincidencia facial' }, { status: 400 });
    }
    
    const result = await handleWorkSchedule(action, matchedEmployee.id, formattedDate);
