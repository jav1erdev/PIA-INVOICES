...
export async function GET(req: NextRequest) {
  if (!io) {
    ...
    io.on('connection', (socket) => {
      ...
      socket.on('send-location', async (truckId, invoiceId, employeeId, location) => {
        console.log(`Recibido: Ubicación para el camión ${truckId}:`, location, `Venta ID: ${invoiceId}`);

        try {
          // Verifica si los parámetros son correctos
          console.log('Parametros de la base de datos:', { truckId, invoiceId, employeeId, location });
          ...
          const result = await sql`
            ...
            RETURNING *;
          `;
          console.log('Ubicación guardada en la base de datos:', result.rows[0]);

          // Emitir la ubicación a todos los clientes
          io?.emit('update-location', { truckId, location });
        } catch (error) {
          console.error('Error guardando ubicación en la base de datos:', error);
        }
      });
